...
import { getFinancialRecords, getFinancialStats } from "@/firebase/financials";
...
export default function Page() {
  ...
  const loadLiveData = async () => {
    try {
      ...
      Object.keys(recordsByYear).forEach((year) => {
        ...
        const monthlyData = Array.from({ length: 12 }, (_, i) => {
          ...
          return {
            ...
            donations: monthIncome, // Assuming all income is donations
          };
        });
        ...
        const programBreakdown = Object.entries(expensesByCategory).map(
          ([name, value], i) => ({
            name,
            value,
            color: ["#1d3557", "#4a5d79", "#77869a", "#a8b5c4"][i % 4],
          })
        );
      ...
      setLiveFinancialData(liveData);
      toast.success("Loaded live financial data from database");
    } catch (error) {
  ...
  const currentFinancialData = useMockData
    ? financialData
    : liveFinancialData || financialData;
  const currentYears = Object.keys(currentFinancialData).sort(
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex justify-center gap-3 mt-8">
              <Button
                ...
                className={cn(
                  !useMockData
                    ? "bg-white text-main-blue hover:bg-white/90"
                    : "border-main-red text-main-red hover:bg-white/10"
                )}
                disabled={loading}
              >
                    ...
                    {(() => {
                      const yearData =
                        currentFinancialData[
                          year as unknown as keyof typeof currentFinancialData
                        ];
                      return (
                        <>
                          {/* Key Metrics */}
